...
//========== Et pour le coup ici je regroupe les fonctions d'appel API de mon panel admin
...
const supprimerProduit = (id) => {
  fetch("http://localhost:80/hellobad-api/supprimer_produit.php", {
    method: "DELETE",
...
const ajouterProduit = () => {
  fetch("http://localhost:80/hellobad-api/ajouter_produit.php", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  .then(() => {
    ...
    setNouveauProduit({
      Nom_produit: "", Description: "", Prix_TTC: "",
      Stock: "", Image_url: "", Marque: "", Est_en_offre: 0, Id_Categorie: ""
    })
  })
}
...
const modifierProduit = () => {
  fetch("http://localhost:80/hellobad-api/modifier_produit.php", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
  ...
  .then(() => {
    setProduits(produits.map(p => 
      p.Id_Produit === produitAModifier.Id_Produit ? produitAModifier : p
    ))
...
useEffect(() => {
    fetch("http://localhost:80/hellobad-api/categories.php")
      .then(res => res.json())
      .then(data => setCategories(data))
  }, [])

  const supprimerCategorie = (id) => {
  fetch("http://localhost:80/hellobad-api/supprimer_categorie.php", {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
...
const ajouterCategorie = () => {
  fetch("http://localhost:80/hellobad-api/ajouter_categorie.php", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
const modifierCategorie = () => {
  fetch("http://localhost:80/hellobad-api/modifier_categorie.php", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
  ...
  .then(() => {
    setCategories(categories.map(c => 
      c.Id_Categorie === categorieAModifier.Id_Categorie ? categorieAModifier : c
    ))
...
useEffect(() => {
    fetch("http://localhost:80/hellobad-api/clients.php")
      .then(res => res.json())
      .then(data => setClients(data))
  }, [])

  const supprimerClient = (id) => {
  fetch("http://localhost:80/hellobad-api/supprimer_client.php", {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
  ...
  .then(res => res.json())
  .then(() => {
    setClients(clients.filter(c => c.ID_Client !== id))
  })
}
...
const ajouterClient = () => {
  fetch("http://localhost:80/hellobad-api/ajouter_client.php", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
const modifierClient = () => {
  fetch("http://localhost:80/hellobad-api/modifier_client.php", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
  ...
  .then(() => {
    setClients(clients.map(c => 
      c.ID_Client === clientAModifier.ID_Client ? clientAModifier : c
    ))
...
useEffect(() => {
  fetch("http://localhost:80/hellobad-api/commandes.php")
    .then(res => res.json())
    .then(data => setCommandes(data))
}, [])

const supprimerCommande = (id) => {
  fetch("http://localhost:80/hellobad-api/supprimer_commande.php", {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      <h1 className="text-2xl font-medium text-gray-900 mb-8">Panel Admin — Produits</h1>
         ...
         <div className="flex gap-2 mb-8">
           ...
           {/* ← ici le contenu selon l'onglet */}
    {onglet === "produits" && (
      <div>
        {/* tout ton code tableau + formulaires produits */}
      </div>
    )}
          ...
          {produits.map((produit) => (
            <tr key={produit.Id_Produit} className="border-b hover:bg-gray-50">
              ...
              <td className="px-4 py-3 flex gap-2">
                <button className="text-blue-500 hover:underline" onClick={() => setProduitAModifier(produit)}>
                  Modifier
                </button>
                <button className="text-red-500 hover:underline" onClick={() => supprimerProduit(produit.Id_Produit)}>
                  Supprimer
                </button>
              </td>
            </tr>
  ...
  <div className="grid grid-cols-2 gap-4">
    ...
    />
    <input
      placeholder="ID Catégorie (1=Raquettes, 2=Shuttlecocks...)"
      value={nouveauProduit.Id_Categorie}
...
{produitAModifier && (
  <div className="mt-10">
    ...
    <div className="flex gap-3 mt-4">
      <button onClick={modifierProduit} className="px-6 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">
        Enregistrer
      </button>
      <button onClick={() => setProduitAModifier(null)} className="px-6 py-2 bg-gray-200 text-gray-700 text-sm rounded-lg hover:bg-gray-300">
        Annuler
      </button>
    </div>
  </div>
...
{onglet === "categories" && (
  <div>
    <h2 className="text-xl font-medium text-gray-900 mb-6">Catégories</h2>
    ...
    <div className="mt-8">
      ...
      <button onClick={ajouterCategorie} className="mt-4 px-6 py-2 bg-gray-900 text-white text-sm rounded-lg hover:bg-gray-700">
        Ajouter
      </button>
    </div>
...
{onglet === "membres" && (
  <div>
    <h2 className="text-xl font-medium text-gray-900 mb-6">Clients</h2>
    ...
    <div className="mt-8">
      ...
      <button onClick={ajouterClient} className="mt-4 px-6 py-2 bg-gray-900 text-white text-sm rounded-lg hover:bg-gray-700">
        Ajouter
      </button>
    </div>
    ...
    {clientAModifier && (
      <div className="mt-8">
        <h3 className="text-lg font-medium mb-4">Modifier le client</h3>
        <div className="grid grid-cols-2 gap-4">
...
{onglet === "commandes" && (
  <div>
    <h2 className="text-xl font-medium text-gray-900 mb-6">Commandes</h2>
    
    <table className="w-full text-sm text-left border border-gray-200">
      <thead className="bg-gray-50">
        <tr>
          <th className="px-4 py-3 border-b">ID</th>
          <th className="px-4 py-3 border-b">Date_commande</th>
          <th className="px-4 py-3 border-b">Statut</th>
          <th className="px-4 py-3 border-b">Total_ttc</th>
          <th className="px-4 py-3 border-b">Adresse_livraison</th>
          <th className="px-4 py-3 border-b">Actions</th>
        </tr>
      </thead>
      <tbody>
